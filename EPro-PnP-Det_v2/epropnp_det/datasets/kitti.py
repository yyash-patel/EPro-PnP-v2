...
class kitti_syn(CustomDataset):
    ...
    def closest_multiple_of_32(self, width):
        lower = (width // 32) * 32
        upper = lower + 32
        return lower if (width - lower) < (upper - width) else upper
    def lower_multiple_of_32(self, width):
        return (width // 32) * 32
    ...
    def load_annotations(self, ann_file):
        ...
        frames = data['frames'][:900]

        data_infos = []
        for frame in frames:
            frame_name = frame['frameName']
            # print(frame_name)
            image_ = cv2.imread(f'/simplstor/ypatel/datasets/KITTI/rendered_images/{frame_name}')
            # print(image_)
            height,width,_ = image_.shape
            
            w_32 = self.lower_multiple_of_32(height)
            crop_box = [0, height-w_32, height, width]
            ...
            for j,car in enumerate(cars):
                ...
                bbox_2d_xyxy = [min(x_values), 
                                min(y_values)-crop_box[1], 
                                max(x_values), 
                                max(y_values)-crop_box[1]]
